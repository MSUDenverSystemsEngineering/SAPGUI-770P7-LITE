...
Try {
	...
	[string]$appName = 'GUI'
	[string]$appVersion = '770 P7 Lite'
	[string]$appArch = 'x86'
	[string]$appLang = 'EN'
	[string]$appRevision = '01'
	[string]$appScriptVersion = '1.0.0'
	[string]$appScriptDate = '08/01/2022'
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		Show-InstallationWelcome -CloseApps 'saplogon, sapsettingsshow' -CheckDiskSpace -PersistPrompt

		## Show Progress Message (with the default message)
		Show-InstallationProgress

		## <Perform Pre-Installation tasks here>
		If(Test-Path "${Env:ProgramFiles(x86)}\SAP\SAPsetup\setup\NwSapSetup.exe"){
			$exitCode = Execute-Process -Path "${Env:ProgramFiles(x86)}\SAP\SAPsetup\setup\NwSapSetup.exe" -Parameters "/uninstall /all /silent"
		...
		If (Test-Path "$env:Public\Desktop\SAP Logon.lnk") {
			Remove-Item "$env:Public\Desktop\SAP Logon.lnk" -Force
		}
		Else {
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		Show-InstallationWelcome -CloseApps 'saplogon, sapsettingsshow' -CloseAppsCountdown 60
